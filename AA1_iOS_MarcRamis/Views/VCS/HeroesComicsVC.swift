...
class HeroesComicsVC : UICollectionViewController {
    ...
    override func viewDidLoad()
    {
        ...
        if let CurrentHero = CurrentHero{
            GetMoreComics(hero: CurrentHero)
        }
    }
}

extension HeroesComicsVC
{
...
extension HeroesComicsVC
{
    func GetMoreComics(hero : Hero) {
        
        if (!GetComicsInProgress)
        {
            GetComicsInProgress = true
            
            Api.Marvel.GetContent(page: .Comics, characterId: hero.id, offset: items.count, limit: 30) {
                responseItems in
