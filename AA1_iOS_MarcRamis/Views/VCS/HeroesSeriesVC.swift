...
class HeroesSeriesVC : UICollectionViewController {
    ...
    override func viewDidLoad()
    {
        ...
        if let CurrentHero = CurrentHero{
            GetMoreSeries(hero: CurrentHero)
        }
    }
}

extension HeroesSeriesVC
{
...
extension HeroesSeriesVC
{
    func GetMoreSeries(hero : Hero) {
        
        if (!GetComicsInProgress)
        {
            GetComicsInProgress = true
            
            Api.Marvel.GetContent(page: .Series, characterId: hero.id, offset: items.count, limit: 30) {
                responseItems in
